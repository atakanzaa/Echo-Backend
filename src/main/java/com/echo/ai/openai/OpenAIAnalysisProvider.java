...
public class OpenAIAnalysisProvider implements AIAnalysisProvider {
    ...
    public OpenAIAnalysisProvider(AppProperties props,
    ...
    @Override
    @CircuitBreaker(name = "openai-analysis", fallbackMethod = "analyzeFallback")
    ...
    @Override
    @CircuitBreaker(name = "openai-analysis", fallbackMethod = "verifyGoalMatchFallback")
    public GoalMatchDecision verifyGoalMatch(GoalMatchVerificationRequest request) {
        Map<String, Object> requestBody = Map.of(
                ...
                "response_format", Map.of("type", "json_object"),
                "messages", List.of(
                        Map.of("role", "system", "content", GOAL_MATCH_SYSTEM_PROMPT),
                        Map.of("role", "user", "content", buildGoalMatchPrompt(request))
                )
        );
        ...
        return parseGoalMatchDecision(extractContent(response.getBody()));
    }
